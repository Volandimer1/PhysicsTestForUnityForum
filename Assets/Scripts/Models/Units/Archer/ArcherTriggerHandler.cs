...
public class ArcherTriggerHandler : MonoBehaviour
{
    ...
    {
        Debug.Log($"{gameObject.name} OnTriggerEnter2D called by {target.name}");
        if (!_targets.Contains(target))
        {
            _targets.Add(target);

            string logText = "List of Targets = ";
    ...
    {
        Debug.Log($"{gameObject.name} OnTriggerExit2D called by {target.name}");
        if (_targets.Contains(target))
        {
